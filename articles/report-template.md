...
`report_assessment.Rmd` in a directory `markdown`, so to use it, you’ll
need to use [`system.file()`](https://rdrr.io/r/base/system.file.html)
...
expression (just like with `plot_assessment`) to select which individual
assessments you want reports on. The reports are written as HTML files
...
document from another template. You can also control the output format,
generating PDF, for example.
