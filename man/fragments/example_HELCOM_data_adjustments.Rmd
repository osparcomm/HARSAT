---
title: "HELCOM data adjustments"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{HELCOM data adjustments}
...
```{r helcom-data-adjust-water_matrix1}
...
```{r helcom-data-adjust-water_dup1}
...
```{r helcom-data-adjust-water_dup2}
wk_rep <- wk$replicate
...
```{r helcom-data-adjust-water_filter1}
water_data$data %>% 
...
```{r helcom-data-adjust-water_filter2}
wk <- filter(
  water_data$data, 
  country != "Russia",
...
```{r helcom-data-adjust-water_filter4}
wk <- wk %>% 
...
```{r helcom-data-adjust-water_filter5}
wk_id <- wk1 %>% 
  filter(type == "mixture") %>% 
  pull(station_name)
...
```{r helcom-data-adjust-water_zero1}
water_data$data <- mutate(
  water_data$data, 
  .id = value == 0
)
...
```{r helcom-data-adjust-water_zero2}
water_data$data <- mutate(
  water_data$data, 
...
```{r helcom-data-adjust-sed_summary1}
wk <- filter(
  sediment_data$data, 
...
```{r helcom-data-adjust-sed_station1}
wk_data <- filter(
  sediment_data$data,
...
```{r helcom-data-adjust-sed_station2}
wk_data %>% 
...
```{r helcom-data-adjust-sed_matrix1}
wk <- sediment_data$data %>% 
  filter(
    !is.na(station_name), 
    !grepl("GSMF", determinand)
...
```{r helcom-data-adjust-sed_matrix2}
wk %>% 
...
```{r helcom-data-adjust-sed_matrix3}
...
```{r helcom-data-adjust-sed_matrix4}
wk %>% 
...
```{r helcom-data-adjust-matrix_Germany_1}
sediment_data$data <- sediment_data$data %>%
  mutate(
...
```{r helcom-data-adjust-matrix_Germany_2}
...
```{r helcom-data-adjust-matrix_Germany_3}
...
```{r helcom-data-adjust-matrix_Germany_4}
...
```{r helcom-data-adjust-sed_PBDE1}
wk <- sediment_data$data %>%
...
```{r helcom-data-adjust-sed_basis1}
wk <- sediment_data$data %>% 
  filter(
    !grepl("GSMF", determinand), 
    determinand != "DRYWT%", 
...
```{r helcom-data-adjust-sed_basis2}
wk %>% 
  filter(basis == "W") %>% 
  with(table(determinand, year, country)) %>% 
  print(zero.print = ".")
...
```{r helcom-data-adjust-sed_depth1}
sediment_data$data %>% 
  filter(
    country == "Poland"
  ) %>% 
...
to 5% CORG is equivalent to normalising to `r round(5/0.35, 1)`% LOIGN.
 
```{r helcom-data-adjust-sed_corg}
lattice::xyplot(
  value ~ year | country, 
...
```{r helcom-data-adjust-sed_corg2}
wk <- sediment_data$data %>% 
  filter(!is.na(station_name)) %>% 
  group_by(station_name) %>% 
  filter(any(year >= 2016)) %>% 
...
```{r helcom-data-adjust-sed_SweCORG1}
...
```{r helcom-data-adjust-sed_setup}
...
```{r helcom-data-adjust-sed_SweCORG2}
wk_new <- 
  read.csv(
...
```{r helcom-data-adjust-sed_SweCORG3}
wk_data <- filter(
  sediment_data$data,
  country == "Sweden",
  year == 2020
...
```{r helcom-data-adjust-sed_SweCORG4}
...
`r if (wk_data_format == "new") "This section is only actioned for the old data format - the issue has been resolved for the new format. "` There are a few instances of multiple method_extraction for sediment in the QA file. These are unusual. If they are single methods, new method_extraction codes should be set up. Unfortunately, the R code can't currently deal with them, so we have to modify them sensibly. Here are the methods and the determinands they are used for (relevant to the HELCOM assessment)

```{r helcom-data-adjust-sed_metcx1, eval = wk_data_format == "old"}
...
```{r helcom-data-adjust-sed_metcx2, eval = wk_data_format == "old"}
...
```{r helcom-data-adjust-sed_uncrt1}
...
```{r helcom-data-adjust-sed_uncrt2}
out_relative <- wk %>%
  filter(
...
```{r helcom-data-adjust-bio_summary1}
wk <- filter(
  biota_data$data,
...
```{r helcom-data-adjust-bio_dup1a}
wk_rep <- wk$replicate
...
```{r helcom-data-adjust-biota_dup2}
...
```{r helcom-data-adjust-bio_dup3a}
wk_rep <- wk$replicate
...
```{r helcom-data-adjust-biota_PCB1}
wk <- biota_data$data %>%
  filter(
    !is.na(station_name),
...
```{r helcom-data-adjust-biota_PCB2}
...
```{r helcom-data-adjust-biota_PCB3}
wk_check <- wk %>%
  group_by(station_name) %>%
  filter(any(year >= 2016)) %>%
  ungroup()
...
```{r helcom-data-adjust-biota_rep1}
relevant_biota <- get_relevant_biota()
...
```{r helcom-data-adjust-biota_rep2}
wk_id <- wk$sub.sample
...
```{r helcom-data-adjust-biota_rep3}
wk_id <- wk %>%
...
```{r helcom-data-adjust-bio_CB153_1}
...
```{r helcom-data-adjust-bio_CB153_2}
...
```{r helcom-data-adjust-bio_censoring1}
get_relevant_biota() %>%
...
```{r helcom-data-adjust-bio_censoring1a}
biota_data$data <- mutate(
  biota_data$data, 
  .id = grepl(">", censoring),
...
```{r helcom-data-adjust-bio_matrix1}
relevant_biota <- get_relevant_biota()
...
```{r helcom-data-adjust-bio_matrix1a}
relevant_biota %>%
...
```{r helcom-data-adjust-bio_matrix2}
wk <- relevant_biota %>%
  filter(
    country == "Finland",
...
```{r helcom-data-adjust-bio_matrix3}
wk %>%
...
```{r helcom-data-adjust-bio_matrix4}
wk %>%
...
```{r helcom-data-adjust-bio_DK_station}
biota_data$data <- mutate(
  biota_data$data,
...
```{r helcom-data-adjust-bio_PBDE1}
wk <- biota_data$data %>%
  filter(grepl("BD", determinand)) %>%
  group_by(station_name) %>%
  filter(any(year >= 2016)) %>%
