...
if (is_normalised) {
  info$unit_text <- paste(info$unit_text, "normalised to", normaliser) 
}
...
This report provides details of the assessment of `r paste(info$det_name, txt_concentrations)` in `r intro_txt` at station `r info$station_name`. `r if (intro_AMAP) intro_txt2``r if (is_normalised) paste0("Concentrations are normalised to ", normaliser, " unless otherwise stated. ")`
...
<li class = "gap">Auxiliary data.  The individual concentration measurements`r if (is_normalised) paste0(", both non-normalised and normalised to ", normaliser, ",")` with supporting information: aluminium content (%) of the sample; `r if (info$group %in% "Metals") "lithium" else "carbon"` content (%) of the sample.</li>
```
