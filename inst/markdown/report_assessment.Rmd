...
# matrix is not in the water timeSeries structure - all assumed to be WT
...
wk_basis <- if (info$compartment == "water") NA_character_ else info$basis

info$unit_text <- label_unit(
  unit = info$unit, 
  basis = wk_basis, 
  html = TRUE
)

# this gets a bit clunky and can surely be streamlined
#
# info$normalise = TRUE means that there was some normalisation in the  
# assesment
# 
# info$is_normalised = TRUE means that this timeseries was normalised
# 
# need to remember that also need to pass such information to multiplot where
# there might be some normalisation, even if the timeseries of interest isn't
# normalised (unlikely I know)

info$is_normalised <- info$normalise && !is.na(info$normaliser)

if (info$is_normalised) {
  
  info$normaliser_text <- label_normaliser(
    info$normaliser, 
    info$normaliser_value, 
    info$normaliser_unit, 
    html = TRUE
  )
  
  info$normaliser_text <- paste("normalised to", info$normaliser_text)

  info$unit_text <- paste(info$unit_text, info$normaliser_text)
}
...
multi_names <- row.names(assessment_object$timeSeries)

info_multi$matrix <- switch(
  info$compartment, 
  biota = assessment_object$timeSeries$matrix,
  sediment = assessment_object$timeSeries$matrix,
  water = rep("WT", nrow(assessment_object$timeSeries))
)
names(info_multi$matrix) <- multi_names

if (info$compartment == "water") {
  info_multi$filtration <- assessment_object$timeSeries$filtration
  names(info_multi$filtration) <- multi_names
}  

info_multi$basis <- assessment_object$timeSeries$basis
names(info_multi$basis) <- multi_names

if (info$normalise) {

  info_multi$normaliser <- assessment_object$timeSeries$normaliser
  names(info_multi$normaliser) <- multi_names
  
  info_multi$normaliser_value <- assessment_object$timeSeries$normaliser_value
  names(info_multi$normaliser_value) <- multi_names
  
  info_multi$normaliser_unit <- assessment_object$timeSeries$normaliser_unit
  names(info_multi$normaliser_unit) <- multi_names
...
This report provides details of the assessment of `r paste(info$det_name, txt_concentrations)` in `r intro_txt` at station `r info$station_name`. `r if (intro_AMAP) intro_txt2``r if (info$is_normalised) paste0("Concentrations are ", info$normaliser_text, " unless otherwise stated. ")`
...
<li class = "gap">Auxiliary data.  The individual concentration measurements`r if (info$is_normalised) paste0(", both non-normalised and ", info$normaliser_text, ",")` with supporting information: aluminium content (%) of the sample; `r if (info$group %in% "Metals" && !(info$is_normalised && info$normaliser %in% "CORG")) "lithium" else "carbon"` content (%) of the sample.</li>
```
...
plot_data(data, assessment, info, assessment_object$info, type = "index", xykey.cex = 1.4)
```
...
```{r, eval = ok, include = FALSE}
auxiliary_id <- switch(
  info$compartment, 
  biota = c("concentration", "LNMEA", "DRYWT%", "LIPIDWT%"), 
  sediment = {
    out <- c("value", "concentration", "AL")
    if (info$group %in% "Metals" && !(info$is_normalised && info$normaliser %in% "CORG")) {
      out <- c(out, "LI") 
    } else {
      out <- c(out, "CORG")
    }
...
auxiliary_missing <- sapply(auxiliary_id, function(x) is.null(data[[x]]))
ok <- ok & !any(auxiliary_missing)
```

```{asis, eval = !ok}
<br>
Not all required auxiliary variables are present. This is an area under development. Please contact the harsat development team.
```
