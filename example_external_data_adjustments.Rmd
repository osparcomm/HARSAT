...
wk_data <- biota_data$data %>% 
  filter(
    country == "Ireland",
    is.na(station_name),
    year %in% biota_data$info$recentYears
  ) 
...
wk_data <- biota_data$data %>% 
  ...
  mutate(
    ...
    value = convert.units(value, from = unit, to = "ug/kg")
  )
...
wk_data %>%  
  with(table(methodUncertainty, year, useNA = "ifany")) %>% 
  print(zero.print = ".")

biota_data$data <- mutate(
  biota_data$data, 
  methodUncertainty = if_else(.id, "U2", methodUncertainty)
)
...
if (!all(na.omit(wk_data$methodUncertainty) %in% "U2"))
  stop("something has changed - investigate")
...
biota_data$data <- mutate(
  ...
  .id = .id & 
    determinand %in% "HG" & 
    basis %in% "D",
  value = convert.basis(value, basis, "W", wk_dw, "", exclude = !.id),
  uncertainty = convert.basis(uncertainty, basis, "W", wk_dw, "", exclude = !.id),
...
wk_data <- biota_data$data %>% 
  ...
  mutate(value = convert.units(value, unit, "ug/kg"))
...
wk_data <- mutate(
  wk_data,
  value = convert.basis(
    value, basis, rep("W", wk_n),
    dry_weight, rep("", wk_n),
    lipid_weight, rep("", wk_n)
  )
)
...
biota_data$data$AMAP_group <- "Not_applicable"
...
biota_data$data <- filter(
  biota_data$data,
  !(country %in% "Denmark" & 
      region %in% "Greenland-Scotland ridge" & 
      rlabo %in% "HFSF"
  )
) 
